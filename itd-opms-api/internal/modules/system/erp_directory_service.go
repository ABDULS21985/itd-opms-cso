...
func (s *ERPDirectoryService) applyRoleBindings(ctx context.Context, tx pgx.Tx, tenantID uuid.UUID, prepared preparedERPDirectory, orgIDs, employeeIDs map[string]uuid.UUID, grantedBy uuid.UUID) (int, error) {
	...
	for _, rec := range prepared.Employees {
		...
		changeRoleMaps := []struct {
			...
			{"release_management_lead", prepared.ReleaseManagementLeads},
			{"solutions_delivery_specialist", prepared.SolutionsDeliverySpecialists},
			{"senior_release_management_specialist", prepared.SeniorReleaseManagementSpecialists},
			{"test_manager", prepared.TestManagers},
			{"solution_developer", prepared.SolutionDevelopers},
			{"senior_solution_developer", prepared.SeniorSolutionDevelopers},
			{"test_analyst", prepared.TestAnalysts},
			{"assistant_it_facilities_specialist", prepared.AssistantITFacilitiesSpecialists},
			{"it_facilities_specialist", prepared.ITFacilitiesSpecialists},
			{"senior_it_facilities_specialist", prepared.SeniorITFacilitiesSpecialists},
			{"it_facilities_lead", prepared.ITFacilitiesLeads},
			{"inventory_officer", prepared.InventoryOfficers},
			{"asset_disposal_committee", prepared.AssetDisposalCommitteeMembers},
			{"ditd_approver", prepared.DITDApprovers},
...
func (s *ERPDirectoryService) insertStagingRows(ctx context.Context, tx pgx.Tx, tenantID, runID uuid.UUID, prepared preparedERPDirectory, _ map[string]uuid.UUID) error {
	for _, rec := range prepared.Employees {
		roleFlags := map[string]bool{
			...
			"release_management_lead":              hasKey(prepared.ReleaseManagementLeads, rec.EmployeeNumber),
			"solutions_delivery_specialist":        hasKey(prepared.SolutionsDeliverySpecialists, rec.EmployeeNumber),
			"senior_release_management_specialist": hasKey(prepared.SeniorReleaseManagementSpecialists, rec.EmployeeNumber),
			"test_manager":                         hasKey(prepared.TestManagers, rec.EmployeeNumber),
			"solution_developer":                   hasKey(prepared.SolutionDevelopers, rec.EmployeeNumber),
			"senior_solution_developer":            hasKey(prepared.SeniorSolutionDevelopers, rec.EmployeeNumber),
			"test_analyst":                         hasKey(prepared.TestAnalysts, rec.EmployeeNumber),
			"assistant_it_facilities_specialist":   hasKey(prepared.AssistantITFacilitiesSpecialists, rec.EmployeeNumber),
			"it_facilities_specialist":             hasKey(prepared.ITFacilitiesSpecialists, rec.EmployeeNumber),
			"senior_it_facilities_specialist":      hasKey(prepared.SeniorITFacilitiesSpecialists, rec.EmployeeNumber),
			"it_facilities_lead":                   hasKey(prepared.ITFacilitiesLeads, rec.EmployeeNumber),
			"inventory_officer":                    hasKey(prepared.InventoryOfficers, rec.EmployeeNumber),
			"asset_disposal_committee":             hasKey(prepared.AssetDisposalCommitteeMembers, rec.EmployeeNumber),
			"ditd_approver":                        hasKey(prepared.DITDApprovers, rec.EmployeeNumber),
