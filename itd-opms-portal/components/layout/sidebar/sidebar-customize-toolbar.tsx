...
export function SidebarCustomizeToolbar({
  ...
  return (
    <motion.div
      ...
      className="px-3 pt-2 pb-1 flex-shrink-0 border-b border-[#A8893D]/15"
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Pencil size={12} className="text-[#C4A962]" />
          <span className="text-xs font-semibold text-[#C4A962]">
            Customizing...
          </span>
          {totalHiddenCount > 0 && (
            <span className="text-[10px] bg-[#A8893D]/20 text-[#C4A962] px-1.5 py-0.5 rounded-full">
      ...
      <div className="flex items-center gap-2 mb-1">
        <button
          onClick={onDone}
          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#A8893D]/20 text-[#C4A962] text-xs font-medium hover:bg-[#A8893D]/30 transition-colors"
        >
          <Check size={12} />
          Done
        </button>
