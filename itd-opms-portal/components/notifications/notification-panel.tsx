...
import { CheckCheck, Inbox, Settings, X } from "lucide-react";
...
export function NotificationPanel({ open, onOpenChange }: NotificationPanelProps) {
  ...
  const notifications: Notification[] = (notificationsData as any)?.data || notificationsData || [];
  const notificationList = Array.isArray(notifications) ? notifications : [];

  const unread = notificationList.filter((n) => !n.isRead);
  const read = notificationList.filter((n) => n.isRead);
