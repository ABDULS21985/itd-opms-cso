...
import { useReleaseCalendar } from "@/hooks/use-release";
import type { ReleaseCalendarEvent } from "@/types/release";
...
export default function ReleaseCalendarPage() {
  ...
  const { data: events, isLoading } = useReleaseCalendar(
  ...
  const eventsByDate = useMemo(() => {
    const raw = Array.isArray(events) ? events : (events as unknown as { data: ReleaseCalendarEvent[] })?.data ?? [];
    if (!raw.length) return {};
    const map: Record<string, ReleaseCalendarEvent[]> = {};
                ...
                >
                  ...
                  <div className="space-y-1">
                    {dayEvents.slice(0, 3).map((evt, j) => {
                      const meta = TYPE_COLORS[evt.releaseType ?? "minor"] ?? TYPE_COLORS.minor;
                      return (
                        <div
                          key={`${evt.id}-${j}`}
