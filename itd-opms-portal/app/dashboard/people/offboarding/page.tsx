...
const TASK_STATUSES: Record<string, { bg: string; text: string; label: string }> = {
  ...
  in_progress: { bg: "rgba(239, 68, 68, 0.1)", text: "#EF4444", label: "In Progress" },
...
function OffboardingCard({
  ...
  const handleDelete = () => {
    if (confirm("Are you sure you want to delete this offboarding checklist? This action cannot be undone.")) {
        ...
        <div className="p-5 space-y-4">
          ...
          >
            ...
            <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
              Offboarding involves revoking access, collecting equipment, and managing knowledge transfer.
              Ensure all sensitive items are addressed before marking as complete.
            </p>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Enter user UUID or employee ID of departing member"
              value={userId}
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold text-white hover:opacity-90 transition-opacity"
          style={{ backgroundColor: "#EF4444" }}
        >
          <Plus size={14} />
          New Template
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-0)] p-12 text-center">
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1 mb-4">
            Create templates to standardize your offboarding process, including access revocation, equipment collection, and exit procedures.
          </p>
          <button
            type="button"
            onClick={() => setShowCreate(true)}
            className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity"
            style={{ backgroundColor: "#EF4444" }}
          >
            <Plus size={14} />
...
export default function OffboardingPage() {
  ...
  const { data: allChecklistsData, isLoading: allLoading } = useChecklists(1, 100, "offboarding");
  const { data: templates, isLoading: templatesLoading } = useChecklistTemplates("offboarding");
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              Manage access revocation, equipment handover, and exit procedures
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90 self-start sm:self-auto"
          style={{ backgroundColor: "#EF4444" }}
        >
          <Plus size={16} />
          Start Offboarding
                ...
                >
                  ...
                  {statusFilter && (
                    <button
                      ...
                      className="rounded-lg px-3 py-2 text-xs font-medium text-[#EF4444] hover:bg-[var(--surface-1)] transition-colors"
                    >
                      Clear filters
                    </button>
                  )}
