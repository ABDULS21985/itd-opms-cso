...
interface EmployerNotificationBellProps {
  onClick: () => void;
}

export function EmployerNotificationBell({ onClick }: EmployerNotificationBellProps) {
  ...
  return (
    ...
    >
      ...
      />
      {notificationCount > 0 && (
        <span className="absolute top-0.5 right-0.5 min-w-[16px] h-4 flex items-center justify-center rounded-full bg-[var(--accent-orange)] text-white text-[10px] font-bold px-1 animate-in fade-in zoom-in">
...
interface EmployerNotificationPanelProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function EmployerNotificationPanel({ open, onOpenChange }: EmployerNotificationPanelProps) {
            ...
            <div className="px-5 py-4 border-b border-[var(--border)] flex-shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {notificationCount > 0 && (
                    <span className="min-w-[20px] h-5 flex items-center justify-center rounded-full bg-[var(--accent-orange)] text-white text-[10px] font-bold px-1.5">
                      {notificationCount}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  {notificationCount > 0 && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-[var(--accent-orange)] hover:bg-[var(--accent-orange)]/6 disabled:opacity-50 transition-colors"
                    >
                      <CheckCheck size={14} />
                      Mark all read
                    </button>
