...
const variantStyles: Record<BadgeVariant, string> = {
  ...
  info:
    "bg-[var(--info-light)] text-[var(--info-dark)] border-[var(--info)]/20",
  default:
...
function resolveVariant(status: string): BadgeVariant {
  ...
  if (
    ["active", "approved", "completed", "hired", "accepted", "verified", "published", "placed"].includes(s)
  ) {
    return "success";
  }
  if (
    ["pending", "review", "under_review", "in_progress", "draft", "shortlisted"].includes(s)
  ) {
    return "warning";
  }
  if (
    ["rejected", "declined", "failed", "expired", "suspended", "closed", "cancelled"].includes(s)
  ) {
    return "error";
  }
  if (["new", "open", "submitted", "invited", "interview"].includes(s)) {
    return "info";
  }
  return "default";
}
